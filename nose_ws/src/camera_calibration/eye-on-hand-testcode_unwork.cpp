...
// 	-0.3334   ,-0.1250   , 0.8742 , 169.9050  , -8.4884   ,-0.8669,
// 	-0.2097  , -0.1731  , 0.8066  ,156.3121 , -12.2064  ,  2.6940,
// 	0.0877  , -0.2295   , 0.8189 , 144.7155 ,  -6.8333  , -2.4815,
// 	0.3694  , -0.1855  ,  0.9909 , 160.0666  , -4.3398 ,  -7.2672);


// Mat_<double> CalPose_val = (cv::Mat_<double>(1, 6) <<
// 	0.4643  , -0.1339  ,  1.1286  ,174.2637 ,  -9.2446  , -2.5594);


//target2camera is right !

Mat_<double> CalPose = (cv::Mat_<double>(4, 6) <<
    0.1989   ,-0.2883   , 0.8773 ,-169.6619  , -8.2017  , -2.3602,
    0.0636  , -0.4859   , 0.7027 ,-156.2907 , -12.2545  , -2.4624,
   -0.2069   ,-0.6508   , 0.5285 ,-144.1065 ,  -4.1406 ,  -5.9767,
-0.5041  , -0.4571   , 0.8564 ,-159.3143   ,-1.5848  , -8.3014);


Mat_<double> CalPose_val = (cv::Mat_<double>(1, 6) <<
  -0.6485  , -0.2164   , 1.0225 ,-173.7668  , -8.9299  , -3.5090);


// Mat_<double> CalPose = (cv::Mat_<double>(4, 7) <<
// 	 -0.333794238938        ,-0.125101113475        ,0.87510658657        ,0.993172528505        ,-0.013673564332        ,0.0768330814327        ,0.0867066309999,
// 	-0.209766124491      ,  -0.173069004329      ,  0.806657199252      ,  0.973542615952     ,  0.00136620528543      ,  0.10761460748       , 0.201573819395,
// 	 0.0876235223964     ,   -0.229454771822    ,    0.818487021524   ,     0.948414768548     ,   -0.0463361477209    ,    0.0774692935922        ,0.303909356166,
// 	0.369266623699      ,  -0.185483085662       , 0.990705798676      ,  0.980269994247     ,   -0.0732542721459     ,   0.0516251388737      ,  0.176180007459);

// Mat_<double> CalPose_val = (cv::Mat_<double>(1, 7) <<
// 	 0.464355234843      ,  -0.133929276555   ,     1.12865998859     ,   0.994945139014     ,   -0.0256613531288     ,   0.0823290451029     ,   0.0514547727599);
...
Mat_<double> ToolPose = (cv::Mat_<double>(4, 6) <<
	  -0.3000    ,0.1500   ,-0.4000   ,90.0000         ,0      ,   0,
	    -0.2353  ,  0.0447  , -0.4990  , 76.5937   , 4.3330  ,  2.1619,
	 0.0863  , -0.1003  , -0.5837   ,61.2972   , 1.0079   ,-2.0020,
	  0.4270   , 0.0311  , -0.4304  , 75.8928   ,-3.4454  , -4.3580);

Mat_<double> ToolPose_val = (cv::Mat_<double>(1, 6) <<
	  0.5000   , 0.1500  , -0.3000  , 90.0000    ,     0    ,     0);	 
...
int main(int argc, char** argv)
{
	...
	//TSAI计算速度最快
	cout << R_gripper2base[0] << endl;
	cout << T_gripper2base[0] << endl;
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	**************************************************/
	...
	cout << "2.5 : " << Homo_gripper2base[3] * Homo_cam2gripper * Homo_target2cam[3] << endl;	

	cout << "Toolpose val : " << Homo_gripper2base_val[0] * Homo_cam2gripper * Homo_target2cam_val[0] << endl;	
	//标定板在相机中的位姿
